...
<?php endif; ?>
<?php $component->withAttributes(['href' => '/patient/dashboard','active' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(false),'icon' => '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>']); ?>Dashboard <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['href' => '/patient/search','active' => true,'icon' => '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>']); ?>Find Doctors <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginalc295f12dca9d42f28a259237a5724830)): ?>
<?php $attributes = $__attributesOriginalc295f12dca9d42f28a259237a5724830; ?>
<?php unset($__attributesOriginalc295f12dca9d42f28a259237a5724830); ?>
...
<div class="max-w-3xl space-y-5">
    ...
    <div class="bg-white rounded-2xl shadow border border-slate-100 p-6">
        <div class="flex flex-col sm:flex-row gap-5">
            <div class="flex-shrink-0">
                <?php if($doctor->doctorProfile?->doctor_image): ?>
                    <img src="<?php echo e(asset('storage/' . $doctor->doctorProfile->doctor_image)); ?>"
            ...
            <div class="flex-1">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <h1 class="text-2xl font-bold text-slate-900">Dr. <?php echo e($doctor->name); ?></h1>
                        <p class="text-brand-600 font-semibold"><?php echo e($doctor->doctorProfile?->specialization); ?></p>
                        <div class="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-slate-400">
                            ...
                            <span><?php echo e($doctor->doctorProfile->experience); ?> yrs exp</span>
                            <?php endif; ?>
                            <?php if($doctor->phone): ?>
                    ...
                    <div class="text-right">
                        <p class="text-3xl font-bold text-slate-900">₹<?php echo e(number_format($doctor->doctorProfile?->consultation_fee ?? 0)); ?></p>
                        <p class="text-xs text-slate-400">per visit</p>
                        <div class="flex items-center gap-1 mt-1 justify-end">
                            <?php for($i=1;$i<=5;$i++): ?>
                                <svg class="w-3.5 h-3.5 <?php echo e($i <= round($doctor->doctorProfile?->rating ?? 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-200'); ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                            <?php endfor; ?>
                            <span class="text-xs text-slate-400 ml-1">(<?php echo e($doctor->doctorProfile?->review_count ?? 0); ?>)</span>
                        </div>
                    </div>
                </div>
                <?php if($doctor->doctorProfile?->bio): ?>
                <p class="text-slate-500 text-sm mt-4 leading-relaxed"><?php echo e($doctor->doctorProfile->bio); ?></p>
                <?php endif; ?>
                <?php if($doctor->doctorProfile?->qualifications): ?>
                <div class="flex flex-wrap gap-2 mt-3">
                    <?php $__currentLoopData = $doctor->doctorProfile->qualifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-brand-100 text-brand-700"><?php echo e($q); ?></span>
    ...
    <div class="bg-orange-50 border border-orange-200 rounded-2xl p-6">
        ...
        <form method="POST" action="/patient/doctors/<?php echo e($doctor->id); ?>/book" class="space-y-4">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="appointment_date" value="<?php echo e(session('full_date', now()->format('Y-m-d'))); ?>">
    ...
    <div class="bg-white rounded-2xl shadow border border-slate-100 p-6">
        ...
        <div class="flex gap-2 overflow-x-auto pb-2 mb-5">
            ...
            <a href="?date=<?php echo e($d); ?>"
               class="flex-shrink-0 flex flex-col items-center px-4 py-3 rounded-xl border-2 transition-all duration-150 min-w-[64px] text-center
                      <?php echo e($d === $selectedDate ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'); ?>">
                <span class="text-[11px] font-medium uppercase"><?php echo e(\Carbon\Carbon::parse($d)->format('D')); ?></span>
                <span class="text-xl font-bold"><?php echo e(\Carbon\Carbon::parse($d)->format('j')); ?></span>
                <span class="text-[11px]"><?php echo e(\Carbon\Carbon::parse($d)->format('M')); ?></span>
            </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        ...
        <?php else: ?>
            <form method="POST" action="/patient/doctors/<?php echo e($doctor->id); ?>/book">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-3 sm:grid-cols-5 gap-2 mb-6">
                    ...
                    <label class="cursor-pointer">
                        <input type="radio" name="slot_id" value="<?php echo e($slot->id); ?>" class="sr-only peer" required>
                        <div class="py-2.5 rounded-xl text-sm font-medium border-2 text-center transition-all
                            ...
                            <?php echo e(\App\Helpers\AppHelper::formatTime($slot->start_time)); ?>
        ...
        <div class="mb-5 p-4 rounded-xl <?php echo e($isFull ? 'bg-red-50 border border-red-200' : 'bg-teal-50 border border-teal-200'); ?>">
            <div class="flex items-center justify-between">
                <div>
                    <p class="font-semibold text-sm <?php echo e($isFull ? 'text-red-700' : 'text-teal-700'); ?>">
                        <?php echo e($isFull ? '❌ Fully Booked for Selected Date' : '🎫 Token Queue Open'); ?>

                    </p>
                    <p class="text-xs mt-0.5 <?php echo e($isFull ? 'text-red-500' : 'text-teal-600'); ?>">
                        <?php echo e($tokenCount ?? 0); ?> / <?php echo e($maxTokens); ?> tokens taken
                        <?php if(!$isFull): ?> · <?php echo e($tokensLeft); ?> remaining <?php endif; ?>
                    </p>
                </div>
                <?php if(!$isFull): ?>
                <div class="text-right">
                    <p class="text-2xl font-bold text-teal-700"><?php echo e(($tokenCount ?? 0) + 1); ?></p>
            ...
            <div class="mt-3 w-full bg-white rounded-full h-2 overflow-hidden border border-slate-100">
                <div class="h-2 rounded-full transition-all <?php echo e($isFull ? 'bg-red-400' : 'bg-teal-400'); ?>"
                     style="width: <?php echo e(min(100, round((($tokenCount ?? 0) / max(1, $maxTokens)) * 100))); ?>%"></div>
            </div>
        </div>
        ...
        <form method="POST" action="/patient/doctors/<?php echo e($doctor->id); ?>/book">
            ...
            <div class="mb-5">
                ...
                <div class="flex gap-2 overflow-x-auto pb-2">
                    ...
                    <label class="cursor-pointer flex-shrink-0">
                        <input type="radio" name="appointment_date" value="<?php echo e($d); ?>"
                            <?php echo e($d === $selectedDate ? 'checked' : ''); ?>
            ...
            <div class="flex items-center gap-4">
                <button type="submit"
                    ...
                    🎫 Get Token #<?php echo e(($tokenCount ?? 0) + 1); ?>
        ...
        <div class="mt-4 p-4 bg-orange-50 border border-orange-200 rounded-xl">
            ...
            <form method="POST" action="/patient/doctors/<?php echo e($doctor->id); ?>/book" class="space-y-3">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="appointment_date" value="<?php echo e($selectedDate); ?>">
                <input type="hidden" name="request_extra" value="1">
        ...
        <div class="space-y-4">
            ...
            <div class="pb-4 border-b border-slate-100 last:border-0 last:pb-0">
                <div class="flex items-center justify-between mb-1">
                    <span class="text-sm font-semibold text-slate-800"><?php echo e($review->patient->name); ?></span>
                ...
                <p class="text-sm text-slate-500 leading-relaxed"><?php echo e($review->comment); ?></p>
                <p class="text-xs text-slate-400 mt-1"><?php echo e($review->created_at->format('M d, Y')); ?></p>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Downloads\docbook1_final\docbook1\docbook1\resources\views/patient/doctor-profile.blade.php ENDPATH**/ ?>